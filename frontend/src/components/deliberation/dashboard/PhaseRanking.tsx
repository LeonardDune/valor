...
export const PhaseRanking = ({ participants, onAdvance, canAdvance, isAdvancing, validationMessage }: PhaseRankingProps) => {
    const completed = participants.filter(p => p.has_completed_ranking || p.has_ranking).length;
    ...
    return (
        <div className="space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
                <Card>
                    <CardHeader>
                        <CardTitle>Voortgang Prioritering</CardTitle>
                        <CardDescription>Deelnemers categoriseren de claims.</CardDescription>
                    </CardHeader>
                                    ...
                                    <div key={p.user_id} className="flex items-center justify-between p-2 rounded-md bg-muted/30">
                                        ...
                                        {(p.has_completed_ranking || p.has_ranking) ? (
                                            <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded flex items-center gap-1">
                                                <CheckCircle className="h-3 w-3" /> Klaar
                                            </span>
                                        ) : (
