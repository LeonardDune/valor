...
export const AgentPanelRegion: React.FC<{ className?: string }> = ({ className }) => {
    return (
        <Panel variant="plain" padding="none" className={cn("flex flex-col h-full border-l border-border-standard bg-background-secondary", className)}>
            <div className="p-4 font-bold text-text-primary border-b border-border-standard">
                Agent Collaboration
            </div>
            <Accordion type="multiple" defaultValue={['proposals']} className="flex-1 overflow-y-auto w-full">

                <AccordionItem value="proposals" className="border-b border-border-subtle">
                    <AccordionTrigger className="w-full flex items-center justify-between p-4 text-sm font-medium text-text-primary hover:bg-white/5 transition-all">
                        Proposals & Activity
